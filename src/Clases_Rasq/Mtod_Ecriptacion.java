 ...
 * @author Works
 */
public class Mtod_Ecriptacion 
{

 private int[] S = new int[256];
 private  int keylen;    
String contra_aleatoria;    
    



  public void keygen(final byte[] key) { 
		if (key.length < 1 || key.length > 256) {
			throw new IllegalArgumentException("la keygen exede 1 a 256 bytes");
		} else {
			keylen = key.length;
			for (int i = 0; i < 256; i++) {
				S[i] = i;
...
contra_aleatoria = cadenaAleatoria;
